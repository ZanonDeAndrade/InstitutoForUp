...
const NewsSection = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.warn("Falha ao carregar posts do blog", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section id="blog" className="bg-gradient-to-b from-background via-card to-background py-20 px-4">
        ...
        <div className="flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2 max-w-2xl">
              Artigos, eventos e comunicados do Blog do Instituto FOR UP.
            </p>
          </div>
          <ButtonLink />
        </div>

        {loading && <p className="mt-8 text-muted-foreground">Carregando artigos do blog...</p>}
